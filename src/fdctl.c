 ...
 */
static int fdctl_wait_single(int fd, int flags)
{
	struct pollfd pfd;
	int ret;
...
again:
	...
	/* POLLERR */
	/* FIXME: Should we return 1 here and force the caller
 ...
 *
 * Failure Modes:
 *  -# undefined: any failure mode of fdctl_wait_single()
